...
% Interpolated thrust extraction
    ...
    % Sugar_motor motor
    thrust_Sugar_motor = interpolatedThrust('/configs/Motors/Sugar_motor.csv',time);  %Thrust Extraction
    std_Sugar_motor = readmatrix('/configs/Motors/Sugar_motor.csv');    %Standard Thrust Samples Extraction
    std_Sugar_motor = [0,0; std_Sugar_motor];
...
% Scatter Plot
    ...
    % Sugar_motor motor
    figure(5)
    p = plot(std_Sugar_motor(:,1),std_Sugar_motor(:,2),'-g'); hold on;
    p.LineWidth = 3;
    scatter(time,thrust_Sugar_motor,10,'.','r');
    legend('Standard','Extracted');
    xlabel('Time');
    ylabel('Thrust');
    title('Thrust Curve Sugar Motor');